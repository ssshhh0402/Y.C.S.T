    ...
    public static void main(String[] args) throws IOException{
        ...
        while(!pq.isEmpty()){
            ...
            for(BJ1446_Pair toGo : base[now.end]){
                if(toGo.end <= D && dist[toGo.end] > dist[now.end] + now.weight){
                    dist[toGo.end] = dist[now.end] + now.weight;
                    pq.add(new BJ1446_Pair(toGo.end, dist[toGo.end]));
                }
            }
            if(now.end < D && dist[D] > dist[now.end] + (D - now.end)){
                dist[D] = dist[now.end] + (D - now.end);
            }
        }
